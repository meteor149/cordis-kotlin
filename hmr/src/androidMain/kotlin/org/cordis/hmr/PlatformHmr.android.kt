...
internal actual object PlatformHmr {
    actual fun resolveBase(configured: String?, baseUrl: String?): String {
        ...
        return normalize(if (value.startsWith("file:")) Paths.get(URI.create(value)).toString() else value)
    }

    actual fun normalize(path: String): String = Paths.get(path).toAbsolutePath().normalize().toString()
    ...
    actual fun relative(base: String, url: String): String = runCatching {
        val target = if (url.startsWith("file:")) Paths.get(URI.create(url)) else Paths.get(url)
        Paths.get(base).relativize(target).toString()
    }.getOrElse { url }
    actual fun readLines(path: String): List<String> = Files.readAllLines(Paths.get(path))
}
